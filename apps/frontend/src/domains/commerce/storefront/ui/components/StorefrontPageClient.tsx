...
export default function StorefrontPageClient({ buId, slug }: { buId: string, slug: string }) {

    const { data: config, isLoading: configLoading } = useGetStoreConfigQuery(buId);
    const { data: page, isLoading: pageLoading, error } = useGetStorePageQuery({ buId, slug });
        ...
        <div style={themeStyle} className="min-h-screen bg-gray-50 text-gray-900 font-sans">
            ...
            <footer className="bg-gray-900 text-white py-10 mt-10">
                <div className="container mx-auto px-4 text-center">
                    <p>{config?.data?.footer?.copyrightText || "© 2025 Powered by Manoxen"}</p>
                </div>
            </footer>
        </div>
    );
