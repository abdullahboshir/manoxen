...
export const ProductGridBlock = ({ data }: { data: any }) => {
  ...
  const buId = (params.businessUnit || params.buId)Id as string;

  const storeId = buId || ((params["business-unit"] || params.buId) as string);
        ...
        <div className="contents storefront-grid-dynamic">
          {isLoading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[3/4] bg-slate-100 animate-pulse rounded"
              ></div>
            ))
          ) : products.length === 0 ? (
            ...
            products.map((p: any) => (
              <Card
                key={p._id || p.id}
                className="hover:shadow-lg transition-shadow cursor-pointer border-none shadow-sm h-full flex flex-col"
              >
...
export const BannerBlock = ({ data }: { data: any }) => {
  if (!data?.image)
    return (
      <div className="p-10 text-center bg-gray-100 border-2 border-dashed m-4">
      ...
      <div className="flex gap-4 mt-4 md:mt-0">
        {["02", "12", "45", "30"].map((num, i) => (
          <div key={i} className="flex flex-col items-center">
            <div className="bg-white text-gray-900 font-bold text-xl px-3 py-2 rounded shadow w-12 text-center">
              {num}
            </div>
