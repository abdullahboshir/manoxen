...
import { useGetProductsQuery } from "@/domains/commerce/catalog/api/productApi";
import { useGetAllUsersQuery } from "@/domains/core/iam/api/userApi";
import { useCreateOrderMutation } from "@/domains/commerce/sales/api/orderApi";
...
export default function CreateOrder() {
  ...
  const { user } = useAuth();
  const { currentBusinessUnit: paramBusinessUnitObj } =
  ...
  // If paramBusinessUnitObj exists (Scoped Context), use it.
  // Otherwise allow Manual Selection (Global Context).
  const contextBusinessUnitId =
    paramBusinessUnitObj?._id || paramBusinessUnitObj?.id;
  const buId = contextBusinessUnitId || selectedBusinessUnitId;
    ...
    useGetProductsQuery(
      {
        search: searchQuery,
        limit: 20,
        businessUnit: buId, // Explicitly use the effective BU ID (scoped or selected)
      },
      {
        // Only skip if IS Super Admin AND NO BU selected (Global context without selection)
        // If in Scoped context, buId will be set, so it won't skip.
        skip: isSuperAdmin && !buId,
      },
    );
  ...
  const products: Product[] = useMemo(() => {
    ...
    if (rawProducts?.results && Array.isArray(rawProducts.results))
      return rawProducts.results;
    return [];
  }, [rawProducts]);
    ...
    try {
      ...
      if (resData) {
        if (
          ...
          Array.isArray(resData.errorSources) &&
          resData.errorSources.length > 0
        ) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-100px)]">
      <div className="md:col-span-2 flex flex-col gap-4">
        ...
        {/* For non-super admins (scoped), we still might need to select outlet if they have multiple? 
                    ...
                    The safer bet is to always show Outlet selector if buId is present but outlet isn't auto-selected.
                */}
        {!isSuperAdmin && buId && (
          <Card className="p-4 border-dashed border-2 bg-muted/20">
            <div className="flex items-center gap-4">
                      ...
                      products.map((product) => (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge
                              variant={
                                product.inventory?.inventory?.stock > 10
                                  ? "secondary"
                                  : "destructive"
                              }
                              className="whitespace-nowrap"
                            >
                              {product.inventory?.inventory?.stock || 0} in
                              stock
                            </Badge>
                          </TableCell>
        ...
        <Card className="flex-1 flex flex-col h-full border-primary/20 shadow-lg">
              ...
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div
                    ...
                    className="flex gap-3 items-start border-b pb-3 last:border-0 animation-fade-in"
                  >
          ...
          <div className="p-4 bg-muted/20 border-t space-y-4">
            ...
            >
              {isSubmitting
                ? "Processing..."
                : `Complete Order (${totalAmount} BDT)`}
            </Button>
          </div>
        </Card>
      </div>
      ...
      <Dialog
        open={!!orderSuccess}
        onOpenChange={(open) => !open && handleNewSale()}
      >
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button
              variant="secondary"
              className="flex-1"
              onClick={() => toast.info("Printing not implemented yet")}
            >
