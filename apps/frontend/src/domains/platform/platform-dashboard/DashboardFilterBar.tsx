...
export function DashboardFilterBar({
  ...
  const [buId, setBusinessUnitId] = useState<string | null>(null);
  ...
  // 1. Fetch organizations (Root Level)
  ...
  const outlets = Array.isArray(outletsData)
    ? outletsData
    : (outletsData as any)?.result || [];

  // Effect: Notify parent on ANY state change
  useEffect(() => {
    onFilterChange({ organizationId, buId, outletId, dateRange: date });
  }, [organizationId, buId, outletId, date, onFilterChange]);
  ...
  const handleOrganizationChange = (val: string) => {
    ...
    router.push(
      `/platform/dashboard${params.toString() ? `?${params.toString()}` : ""}`,
    );
  };
    ...
    <div className="flex flex-col md:flex-row gap-2 items-center bg-card p-2 rounded-lg border shadow-sm">
      ...
      {organizationId && (
        <>
          ...
          <Select onValueChange={handleBuChange} value={buId || "all"}>
            <SelectTrigger className="w-[180px] h-9 text-sm">
