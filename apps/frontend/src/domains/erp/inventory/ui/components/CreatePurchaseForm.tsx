...
import { useCreatePurchaseMutation } from "@/domains/erp/inventory/api/purchaseApi";
import { useGetSuppliersQuery } from "@/domains/commerce/contacts/api/supplierApi";
...
export function CreatePurchaseForm() {
    ...
    const { data: outlets = [] } = useGetOutletsQuery({ businessUnit: buId }, { skip: !buId });
    ...
    useEffect(() => {
        if (buId) {
            form.setValue("businessUnit", buId);
        }
    }, [buId, form]);
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <Select onValueChange={field.onChange} value={field.value} disabled={!buId}>
                                                <FormControl>
                                                    <SelectTrigger>
                        ...
                        <Card className="h-full flex flex-col">
                            ...
                            <CardContent className="flex-1 space-y-4">
                                ...
                                <Popover open={openProductSearch} onOpenChange={setOpenProductSearch}>
                                    <PopoverTrigger asChild>
                                        <Button variant="outline" role="combobox" aria-expanded={openProductSearch} className="w-full justify-between h-12 text-base" disabled={!buId}>
