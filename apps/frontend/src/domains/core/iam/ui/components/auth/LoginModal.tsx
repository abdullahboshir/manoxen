...
import { useUserRegisterMutation } from "@/domains/core/iam/api/authApi";
...
export default function LoginModal({
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      console.log("From LoginModal", res);
      ...
      if (res.redirect) {
        ...
        let roleName = normalizeAuthString(
          (user?.role as any) || (user?.role as any)?.name,
        );
        ...
        if (
        ...
        } else if (user?.businessAccess && user.businessAccess.length > 0) {
          const access = user.businessAccess[0];
          const buSlug =
            typeof access.businessUnit === "object" &&
            access.businessUnit !== null
              ? access.businessUnit.slug ||
                access.businessUnit.id ||
                access.businessUnit._id
              : access.businessUnit;

          const orgSlug =
            typeof access.organization === "object" &&
          ...
          if (buSlug) {
            router.push(`/${orgSlug}/${buSlug}/dashboard`);
          } else {
  ...
  return (
    <Dialog
      open={isLoginPage ? true : open}
      onOpenChange={(o) => !isLoginPage && onOpenChange?.(o)}
    >
      <DialogContent
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-center">
            {activeTab === "login"
              ? "Sign in to your Manoxen POS dashboard"
              : "Create your Manoxen POS account"}
          </DialogDescription>
        </DialogHeader>
            ...
            <TabsContent value="login" className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-primary pointer-events-none opacity-50 cursor-not-allowed">
                    Forgot Password?
                  </span>
                </div>
