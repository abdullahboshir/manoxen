...
export default function BusinessOverview({ context }: { context?: any }) {
  ...
  const buId = buData?._id || buData?.id;
  ...
  const { data: outletData, isLoading: outletsLoading } = useGetOutletsQuery(
    ...
    {
      skip: !buId, // Skip until ID is resolved
    },
  );

  const outlets = Array.isArray(outletData)
    ? outletData
    : outletData?.data?.result || outletData?.result || [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {statsLoading ? "..." : stats?.activeSales || 0}
            </div>
            <p className="text-xs text-muted-foreground">
