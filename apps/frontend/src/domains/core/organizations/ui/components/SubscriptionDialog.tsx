...
import {
  ...
  useGetLicensesQuery,
} from "@/redux/api/platform/licenseApi";
...
import { OrganizationColumn } from '@/domains/core/organizations/ui/components/columns';
...
export function SubscriptionDialog({
  ...
  const [createLicense, { isLoading: isSubmitting }] =
    useCreateLicenseMutation();
  ...
  const pkg = useMemo(
    () => packages?.data?.find((p: any) => p._id === selectedPackage),
  ...
  const { data: licenseData, isLoading: isLoadingLicense } =
    useGetLicensesQuery(
      {
        clientId:
          organization.id ||
          (organization as any)._id ||
          (organization as any).value,
      },
  ...
  const activeLicense = useMemo(() => {
    ...
    if (Array.isArray(responseBody)) {
      ...
      Array.isArray(responseBody.data.result)
    ) {
  ...
  useEffect(() => {
    ...
    if (activeLicense) {
      ...
      if (activeLicense.packageId)
        setSelectedPackage(
          activeLicense.packageId._id || activeLicense.packageId,
        );
      ...
      if (activeLicense.customModules) {
        ...
        Object.entries(activeLicense.customModules).forEach(
          ([key, val]: [string, any]) => {
      ...
      console.log(
        "Prefilling from Organization Data (Fallback):",
        organization,
      );

      // 1. Try to match Package
      if (organization.subscription?.planName && packages?.data) {
        const matchedPkg = packages.data.find(
          (p: any) => p.name === organization.subscription?.planName,
        );
    ...
    const calculatePrice = async () => {
      ...
      try {
        ...
        const response: any = await axiosInstance.post(
          `/platform/licenses/calculate-price`,
          {
    ...
    try {
      ...
      toast.success(
        billingType === "complimentary"
          ? "Complimentary lifetime access granted"
          : "Subscription updated successfully",
      );
      onOpenChange(false);
    } catch (error: any) {
      console.error(
        "❌ Submit Failed. Error Object:",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Update plan and billing status for{" "}
                          ...
                          <div className="flex flex-col text-left">
                            ...
                            <span className="text-[10px] text-muted-foreground">
                              {pkg.billingCycle === "lifetime"
                                ? "One-time Payment"
                                : `${pkg.price.toLocaleString()} ${pkg.currency}/mo`}
                            </span>
                          </div>
                        </SelectItem>
                      ))
            ...
            {pkg && (
              <div className="space-y-3 rounded-lg border border-primary/20 bg-primary/5 p-4 shadow-inner">
                <div className="flex items-center justify-between border-b pb-2">
                  ...
                  <div className="text-right flex flex-col items-end">
                    <div className="flex items-center gap-1.5">
                      ...
                      <div
                        className={cn(
                          "text-lg font-black text-primary leading-none transition-opacity",
                    ...
                    {backendPricing?.priceBreakdown?.modulePrices?.length >
                      0 && (
                ...
                <div className="grid grid-cols-2 gap-2 max-h-[160px] overflow-y-auto pr-2 custom-scrollbar">
                  {Object.entries(pkg.moduleAccess).map(
                    ([key, config]: [string, any]) => {
                      const isErp = key === "erp";
                      return (
                        <div
                          key={key}
                          className="flex items-center justify-between gap-2 p-1.5 transition-colors hover:bg-white/50 rounded-md border border-transparent hover:border-primary/10"
                        >
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              disabled={
                                isErp ||
                                (config.enabled && pkg.slug !== "trial")
                              } // Package base modules are mandatory except in trials, ERP is ALWAYS mandatory
                              onChange={(e) =>
                            ...
                            <label
                              htmlFor={`mod-dl-${key}`}
                              className="text-[11px] font-semibold capitalize cursor-pointer flex items-center gap-1.5"
                            >
                              {key}
                              {isErp && (
                          ...
                          {isErp ? (
                            <span className="text-[9px] font-medium text-muted-foreground bg-gray-100 px-1.5 rounded">
                              Included
                            </span>
                          ) : (
                            ...
                            !config.enabled && (
                              <span
                                className={cn(
                                  "text-[9px] font-bold px-1 rounded bg-emerald-50 text-emerald-600 border border-emerald-100",
                                  !customModules[key] && "opacity-40 grayscale",
                                )}
                              >
                                +{config.monthlyPrice}
                              </span>
              ...
              <div className="grid gap-2">
                ...
                <Select value={status} onValueChange={setStatus}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem
                      value="trial"
                      disabled={billingType === "complimentary"}
                    >
                      Trial
                    </SelectItem>
