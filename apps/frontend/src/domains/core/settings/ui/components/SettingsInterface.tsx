...
import { GeneralSettings } from "@/domains/core/settings/ui/components/GeneralSettings"
import { SalesSettings } from "@/domains/core/settings/ui/components/SalesSettings"
import { PosSettings } from "@/domains/core/settings/ui/components/PosSettings"
import { InventorySettings } from "@/domains/core/settings/ui/components/InventorySettings"
import { ModuleToggleSettings } from "@/domains/core/settings/ui/components/ModuleToggleSettings"
import { OperationsSettings } from "@/domains/core/settings/ui/components/OperationsSettings"
import { HRMSettings } from "@/domains/core/settings/ui/components/HRMSettings"

// PLATFORM SHARED COMPONENTS
import BrandingSettings from "@/domains/core/settings/shared/BrandingSettings"
import SecuritySettings from "@/domains/core/settings/shared/SecuritySettings"
import ComplianceSettings from "@/domains/core/settings/shared/ComplianceSettings"
import LegalSettings from "@/domains/core/settings/shared/LegalSettings"
import InternationalizationSettings from "@/domains/core/settings/shared/InternationalizationSettings"
import MaintenanceSettings from "@/domains/core/settings/shared/MaintenanceSettings"
import CommercialSaaSSettings from "@/domains/core/settings/shared/CommercialSaaSSettings"
import InfrastructureHubSettings from "@/domains/core/settings/shared/InfrastructureHubSettings"

// NEW SHARED COMPONENTS
import FinancialCoreSettings from "@/domains/core/settings/shared/FinancialCoreSettings"
import DocumentGovernanceSettings from "@/domains/core/settings/shared/DocumentGovernanceSettings"
import ReportingSettings from "@/domains/core/settings/shared/ReportingSettings"
import TaxIntelligenceSettings from "@/domains/core/settings/shared/TaxIntelligenceSettings"
import PricingPolicySettings from "@/domains/core/settings/shared/PricingPolicySettings"
import FulfillmentPolicySettings from "@/domains/core/settings/shared/FulfillmentPolicySettings"
import CorporateRegistrySettings from "@/domains/core/settings/shared/CorporateRegistrySettings"
import PrefixPolicySettings from "@/domains/core/settings/shared/PrefixPolicySettings"
import GovernancePolicySettings from "@/domains/core/settings/shared/GovernancePolicySettings"
import ContactSettings from "@/domains/core/settings/shared/ContactSettings"
...
export function SettingsInterface() {
    ...
    const isOrganizationSettings = searchParams.get('context') === 'organization' || pathname?.includes('organization-settings');

    const organizationId = (user as any)?.organizationId || searchParams.get('organization');
    const { data: organizationSettingsData, isLoading: organizationSettingsLoading } = useGetOrganizationSettingsQuery(organizationId, { skip: !isOrganizationSettings || !organizationId });

    // Fetch Business Unit settings
    const { data: buSettingsData, isLoading: buSettingsLoading } = useGetBusinessUnitSettingsQuery(businessUnit, { skip: !businessUnit || !!outletId || isOrganizationSettings });
    ...
    const { data: platformSettingsData, isLoading: platformLoading } = useGetPlatformSettingsQuery(undefined, { skip: !!businessUnit || isOrganizationSettings || !!outletId });
    ...
    if (outletId) {
        ...
        settingsLoading = organizationSettingsLoading;
    }
    ...
    const [updateOrganizationSettings, { isLoading: updateOrganizationLoading }] = useUpdateOrganizationSettingsMutation();
    const [updateOutletSettings, { isLoading: updateOutletLoading }] = useUpdateOutletSettingsMutation();

    const updateLoading = updateBULoading || updatePlatformLoading || updateOrganizationLoading || updateOutletLoading;
    ...
    useEffect(() => {
        ...
        } else if (businessUnit && !isOrganizationSettings) {
            setLocalSettings(buSettingsData ? JSON.parse(JSON.stringify(buSettingsData)) : {});
        } else if (isOrganizationSettings) {
            setLocalSettings(organizationSettingsData ? JSON.parse(JSON.stringify(organizationSettingsData)) : {});
    ...
    const defaultTab = outletId ? "store-identity" : (businessUnit ? "overview" : (isOrganizationSettings ? "branding" : "branding"));
    ...
    const saveSettings = async () => {
        try {
            if (businessUnit) {
                await updateBUSettings({ buId: businessUnit, data: localSettings }).unwrap();
            ...
            <div className="mb-6 flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">
                        {outletId ? "Outlet Settings" : (businessUnit ? "Business Settings" : (isOrganizationSettings ? "Organization Settings" : "Global Settings"))}
                    </h1>
                    <p className="text-muted-foreground mt-1">
                        {outletId ? "Configure settings specific to this outlet" : (businessUnit ? "Manage your Business Unit configuration" : (isOrganizationSettings ? "Manage Organization preferences" : "Manage complete platform configuration"))}
                    </p>
                </div>
                ...
                <Card className="w-full lg:w-auto h-fit shrink-0 p-0 overflow-hidden bg-background sticky top-6">
                    ...
                    <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
                        <TabsList className="flex flex-col h-full w-full justify-start items-stretch p-2 gap-0.5 bg-transparent">
                            ...
                            {!businessUnit && !outletId && !isOrganizationSettings && (
                                <>
