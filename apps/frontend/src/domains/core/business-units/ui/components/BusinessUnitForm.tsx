...
import { useGetAttributeGroupsQuery } from "@/domains/commerce/catalog/api/attributeGroupApi";
...
const formSchema = z.object({
  ...
  activeModules: z
    .record(z.string(), z.boolean().or(z.undefined()))
    .default({}),
...
export function BusinessUnitForm({ slug }: BusinessUnitFormProps = {}) {
  ...
  const [createBusinessUnit, { isLoading: isCreating }] =
    useCreateBusinessUnitMutation();
  const [updateBusinessUnit, { isLoading: isUpdating }] =
    useUpdateBusinessUnitMutation();
  const { data: businessUnits = [], isLoading: loadingBU } =
    useGetBusinessUnitsQuery({}, { skip: !isEditMode });
  const { data: rawOrganizations, isLoading: isLoadingOrganizations } =
    useGetAllOrganizationsQuery({});
  const { data: attributeGroupsData } = useGetAttributeGroupsQuery({});
  const attributeGroups =
    attributeGroupsData?.result || attributeGroupsData || [];
  const { user } = useAuth();
  const isSuperAdmin =
  ...
  const allOrganizations = Array.isArray(rawOrganizations)
    ? rawOrganizations
    : rawOrganizations?.data || rawOrganizations?.result || [];
  ...
  useEffect(() => {
    console.log("BU_FORM_DEBUG:", {
      ...
      rawOrganizationsData: !!rawOrganizations,
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      organization:
        organizationIdFromUrl ||
        (allOrganizations.length === 1 && !isSuperAdmin
          ? allOrganizations[0]._id || allOrganizations[0].id
          : ""),
    },
  });

  const selectedOrganizationId =
    form.watch("organization") || organizationIdFromUrl;
  const parentOrganization = allOrganizations.find(
    (c: any) =>
      c?._id === selectedOrganizationId || c.id === selectedOrganizationId,
  );
  const organizationName = parentOrganization?.name || "Unknown Organization";
  const allowedModules = parentOrganization?.activeModules;
  ...
  useEffect(() => {
    if (watchName) {
      ...
      if (!form.formState.dirtyFields.branding?.name)
  ...
  useEffect(() => {
    if (
      ...
      form.setValue("organization", organizationIdFromUrl);
    }
  }, [isEditMode, organizationIdFromUrl, form]);
  ...
  useEffect(() => {
    if (
      ...
      const singleOrganizationId =
        allOrganizations[0]._id || allOrganizations[0].id;
      form.setValue("organization", singleOrganizationId);
    }
  }, [isEditMode, organizationIdFromUrl, allOrganizations, form]);
  ...
  useEffect(() => {
    if (isEditMode && businessUnit) {
      form.reset({
        ...
        operationalModel:
          businessUnit.operationalModel || BUSINESS_MODEL.RETAIL,
        ...
        branding: {
          ...
          theme: businessUnit.branding?.theme || {
            primaryColor: "#3B82F6",
            secondaryColor: "#1E40AF",
            accentColor: "#F59E0B",
            fontFamily: "Inter",
        ...
        organization:
          businessUnit.organization?._id ||
          businessUnit.organization?.id ||
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const finalOrganizationId =
        values.organization || selectedOrganizationId || organizationIdFromUrl;
      ...
      if (!payload.organization) {
        toast.error(
          "Organization context is missing. Please select a organization first.",
        );
        return;
      }
      ...
      toast.error(
        error?.data?.message ||
          `Failed to ${isEditMode ? "update" : "create"} business unit`,
      );
    }
  };
          ...
          <TabsContent value="basic">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Define the operational structure and industry
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Organization Selection Logic: 
                                        ...
                                        3. If Owner & multiple organizations -> Dropdown
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="none">
                              None (No Dynamic Attributes)
                            </SelectItem>
                            {(attributeGroups as any[]).map((g: any) => (
                        ...
                        <FormDescription className="text-xs">
                          Links dynamic product attributes to this Business Unit
                        </FormDescription>
                      </FormItem>
                    )}
                  />
