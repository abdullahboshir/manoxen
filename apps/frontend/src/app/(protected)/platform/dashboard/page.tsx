...
export default function ContextAwareDashboard() {
    ...
    // View State Logic
    const isPlatformView = isSuperAdmin(currentRole) && !filters.organizationId;
    const isOrganizationView = !!filters.organizationId && !filters.businessUnitId;
    ...
    const { data: organizationStats, isLoading: organizationLoading } = useGetOrganizationDashboardStatsQuery(
        filters.organizationId || "",
        {
            skip: !isOrganizationView,
            refetchOnMountOrArgChange: true
        }
    );
    ...
    const activeStats = isPlatformView ? platformStats : (isOrganizationView ? organizationStats : buStats);
    const activeLoading = isPlatformView ? platformLoading : (isOrganizationView ? organizationLoading : buLoading);
        ...
        <div className="space-y-6">
            ...
            {isPlatformView ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-7 gap-6">
                        <div className="col-span-1 lg:col-span-4">
                            {isOrganizationView ? <TopPerformers title="Top Business Units" /> : <TopPerformers title="Top Outlets" />}
                        </div>
                        <div className="col-span-1 lg:col-span-3">
                            {isOrganizationView ? <SubscriptionOverview /> : <ModuleUsageStats />}
                        </div>
                    </div>
                </>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="col-span-1 lg:col-span-2 bg-card p-6 rounded-xl shadow-sm border border-border">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {isPlatformView && (
                            <button onClick={() => router.push(`/platform/companies/add?organization=${filters.organizationId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-primary bg-primary/5 border border-primary/20 rounded-lg hover:bg-primary/10 transition-colors">
                                <span>Add Organization</span> <Plus className="w-4 h-4" />
                            </button>
                        )}
                        {!isPlatformView && (
                            <button onClick={() => router.push(`/platform/business-units/new?organization=${filters.organizationId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-primary bg-primary/5 border border-primary/20 rounded-lg hover:bg-primary/10 transition-colors">
                                <span>Add Business Unit</span> <Plus className="w-4 h-4" />
                            </button>
                        )}
                        <button onClick={() => router.push(`/platform/user-management/business-users/add?organization=${filters.organizationId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-lg hover:bg-emerald-100 transition-colors">
                            <span>Add staff</span> <Plus className="w-4 h-4" />
                        </button>
                        <button onClick={() => router.push(`/platform/system/audit-logs?organization=${filters.organizationId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-muted-foreground bg-muted/50 rounded-lg hover:bg-muted transition-colors">
