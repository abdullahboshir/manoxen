...
import { SubscriptionDialog } from "../../organizations/SubscriptionDialog";
...
export default function SubscriptionPage() {
    ...
    const { data: rawOrganizations, isLoading: loadingOrganizations } = useGetAllOrganizationsQuery({});
    const [isDialogOpen, setIsDialogOpen] = useState(false);

    // Resolve Organization Context
    const activeOrganization = useMemo(() => {
        const organizations = Array.isArray(rawOrganizations) ? rawOrganizations : (rawOrganizations?.data || rawOrganizations?.result || []);
        if (!organizations || organizations.length === 0) return null;

        if (organizationIdParam) {
            return organizations.find((c: any) => (c._id === organizationIdParam || c.id === organizationIdParam)) || organizations[0];
        }
        // In global context, do not auto-select first organization if no param is provided.
        // Or if we want to show a "Select Organization" state.
        // For now, consistent behavior:
        return organizations[0];
    }, [rawOrganizations, organizationIdParam]);

    // Fetch License
    const { data: licenseData, isLoading: loadingLicense } = useGetLicensesQuery(
        { clientId: activeOrganization?._id || activeOrganization?.id },
    ...
    if (loadingOrganizations || (activeOrganization && loadingLicense)) {
    ...
    if (!activeOrganization) {
        return (
             <div className="flex flex-col items-center justify-center min-h-[400px] text-muted-foreground">
                ...
                <p>You do not appear to have any organization assigned to your account.</p>
            </div>
        );
    }

    const planName = currentLicense?.packageId?.name || activeOrganization.subscription?.planName || "Trial Plan";
    const status = currentLicense?.status || activeOrganization.subscription?.status || "active";
    const nextBilling = currentLicense?.nextBillingDate || activeOrganization.subscription?.endDate;
    ...
    const activeModules = currentLicense?.customModules 
        ...
        : (currentLicense?.packageId?.moduleAccess 
            ...
            : Object.keys(activeOrganization.activeModules || {}).filter(k => activeOrganization.activeModules[k]));
                    ...
                    <CardContent className="space-y-6 relative z-10">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="flex flex-col space-y-1.5">
                                ...
                                <span className="font-semibold text-lg">{activeOrganization.name}</span>
                                <span className="text-xs text-muted-foreground font-mono">{activeOrganization.id}</span>
                            </div>
                            <div className="flex flex-col space-y-1.5">
                                <span className="text-sm text-muted-foreground flex items-center gap-2">
