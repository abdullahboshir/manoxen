...
import { AuthContext } from "@manoxen/auth-client";

export function AuthProvider({ children }: { children: ReactNode }) {
  const router = useRouter();

  const [activeBusinessUnit, setActiveBusinessUnitState] = useState<
    string | null
  >(() => {
  ...
  const hasToken =
    typeof window !== "undefined" ? document.cookie.includes(authKey) : false;
    ...
    if (idOrObj) {
      ...
      const organizationId =
        typeof idOrObj === "object"
          ? idOrObj.organization?._id?.toString() ||
            idOrObj.organization?.id?.toString() ||
            idOrObj.organization?.toString()
          : null;
    ...
    try {
      ...
      if (checkIsSuperAdmin(roles) || decoded?.isSuperAdmin) {
        ...
        if (scope === "ORGANIZATION") {
          ...
          const availableEntry = context.available?.find(
            (a: any) =>
              a.businessUnit?._id === businessUnit?._id ||
              a.businessUnit?.id === businessUnit?.id ||
      ...
      console.error(
        "LOGIN ERROR CAUGHT IN AUTH PROVIDER:",
