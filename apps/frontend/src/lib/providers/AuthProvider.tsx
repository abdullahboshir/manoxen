...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const setActiveBusinessUnit = (idOrObj: string | any | null) => {
        if (idOrObj) {
            ...
            const organizationId = typeof idOrObj === 'object' ? (idOrObj.organization?._id?.toString() || idOrObj.organization?.id?.toString() || idOrObj.organization?.toString()) : null;

            localStorage.setItem('active-business-unit', id);
            if (organizationId && typeof organizationId === 'string' && organizationId !== '[object Object]') {
