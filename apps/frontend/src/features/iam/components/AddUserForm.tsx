...
export default function AddUserForm({ isPlatformUser = false }: AddUserFormProps) {
    ...
    const canSeeBUs = isSuperAdmin || (currentUser?.roles?.some((r: any) => 
        matchesRole(typeof r === 'string' ? r : r.name, [USER_ROLES.ORGANIZATION_OWNER])
    ));
    const { data: rawBusinessUnits = [] } = useGetBusinessUnitsQuery({ organizationId: organizationId || undefined }, { skip: !canSeeBUs });
