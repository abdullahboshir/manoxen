...
export function UserForm({ initialData, mode, onSubmit, isSubmitting, onCancel, apiError, targetScope, initialBusinessUnitSlug }: UserFormProps) {
    ...
    const [selectedOrganizationId, setSelectedOrganizationId] = useState<string>(() => {
        const urlOrganizationId = searchParams.get('organization') || searchParams.get('organizationId');
        ...
        const primaryOrganizationId = (currentUser as any)?.context?.primary?.organization?._id || (currentUser as any)?.organization?._id || (currentUser as any)?.organizationId;
    ...
    useEffect(() => {
        if (!selectedOrganizationId && availableCompanies.length === 1) {
            setSelectedOrganizationId(availableCompanies[0]._id);
        }
    }, [availableCompanies, selectedOrganizationId]);

    useEffect(() => {
        const urlOrganizationId = searchParams.get('organization') || searchParams.get('organizationId');
        if (urlOrganizationId && urlOrganizationId !== selectedOrganizationId) {
    ...
    const { data: rolesData } = useGetRolesQuery({ organization: selectedOrganizationId || undefined }, { refetchOnMountOrArgChange: true, skip: !selectedOrganizationId && !isPlatformUser })
    const { data: businessUnitsData } = useGetBusinessUnitsQuery({ organization: selectedOrganizationId || undefined }, { skip: !selectedOrganizationId && !isPlatformUser })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (availableCompanies.length > 0 && !selectedOrganizationId) { toast.error("Please select a organization"); return; }
        ...
        roleAssignments.forEach(r => {
            ...
            if (r.businessUnit) {
                if (r.outlets.length > 0) r.outlets.forEach(outletId => permissionsPayload.push({ role: r.role, organization: assignmentOrganization, businessUnit: r.businessUnit, outlet: outletId, scope: 'OUTLET', isPrimary: false }));
                else permissionsPayload.push({ role: r.role, organization: assignmentOrganization, businessUnit: r.businessUnit, outlet: null, scope: 'BUSINESS', isPrimary: false, status: 'ACTIVE' });
            } else permissionsPayload.push({ role: r.role, organization: assignmentOrganization, businessUnit: null, outlet: null, scope: 'GLOBAL', isPrimary: false, status: 'ACTIVE' });
        });
        ...
        const payload: any = {
            ...
            organization: selectedOrganizationId || permissionsPayload.find(p => p.organization)?.organization || null,
        };
