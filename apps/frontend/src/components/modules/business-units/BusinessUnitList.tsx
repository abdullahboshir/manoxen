...
export default function BusinessUnitList() {
  ...
  const organizationIdFromUrl =
    searchParams.get("organization-admin") || searchParams.get("organization");
  const { data: rawUnits, isLoading } = useGetBusinessUnitsQuery(
  ...
  const businessUnits = Array.isArray(rawUnits)
    ? rawUnits
    : rawUnits?.data || rawUnits?.result || [];

  const filteredUnits = Array.isArray(businessUnits)
    ? businessUnits.filter((unit: any) => {
        if (!organizationIdFromUrl) return true;
        const buOrganizationId =
          unit.organization?._id || unit.organization?.id || unit.organization;
        return buOrganizationId === organizationIdFromUrl;
      })
    : [];
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm">
            {row.original.organization?.name || (
              <span className="text-red-500 italic">Unlinked</span>
            )}
          </span>
        </div>
      ),
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-1">
          <Badge variant="outline" className="w-fit">
            {row.original.operationalModel || "Retail"}
          </Badge>
          <span className="text-xs text-muted-foreground capitalize">
            {row.original.industry || "General"}
          </span>
        </div>
      ),
    },
            ...
            <DropdownMenuContent align="end">
                ...
                onClick={() => {
                  ...
                  const orgSlug =
                    typeof unit.organization === "object"
                      ...
                      : unit.organization || organizationIdFromUrl; // Use ID string if object missing

                  // Ideally we need org slug. If only ID available, we might have issues if route expects slug.
                  // Assuming route accepts both or slug is standard.
                  const url = orgSlug
                    ? `/${orgSlug}/${identifier}` // Standard Route: /[org]/[bu]
                    : `/${identifier}/overview`; // Fallback (Legacy)
              ...
              <DropdownMenuItem
                onClick={() => {
                  ...
                  const organizationAdminUrl = `/organization/business-units/${identifier}/edit`;
                  ...
                  if (
                    ...
                    const targetUrl = organizationIdFromUrl
                      ? `${organizationAdminUrl}?organization=${organizationIdFromUrl}`
              ...
              <DropdownMenuItem
                onClick={() => {
                  const identifier = unit.slug || unit.id || unit._id;
                  router.push(
                    `/platform/user-management/business-users/add?business-unit=${identifier}`,
                  );
                }}
              >
              ...
              <DropdownMenuItem
                onClick={() => {
                  const identifier = unit.slug || unit.id || unit._id;
                  router.push(
                    `/platform/business-units/${identifier}/outlets/new`,
                  );
                }}
              >
    ...
    <DataPageLayout
      ...
      createAction={{
        ...
        onClick: () => {
          ...
          router.push(
            `/platform/business-units/new${queryString ? `?${queryString}` : ""}`,
          );
        },
      }}
    >
          ...
          <div className="p-4 bg-muted/30 rounded-md">
            ...
            {original.outlets && original.outlets.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const identifier =
                            original.slug || original.id || original._id;
                          router.push(
                            `/platform/user-management/business-users/add?business-unit=${identifier}&outlet=${outlet._id}`,
                          );
                        }}
                      >
              ...
              <div className="text-muted-foreground text-sm italic">
                No outlets found for this business unit.
              </div>
            )}
          </div>
        )}
