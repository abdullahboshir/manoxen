...
export default function BusinessUnitList() {
    ...
    const organizationIdFromUrl = searchParams.get("organization-admin") || searchParams.get("organization");
    const { data: rawUnits, isLoading } = useGetBusinessUnitsQuery(organizationIdFromUrl ? { organization: organizationIdFromUrl } : {}) as any;
    ...
    const filteredUnits = Array.isArray(businessUnits) ? businessUnits.filter((unit: any) => {
        ...
        const buOrganizationId = unit.organization?._id || unit.organization?.id || unit.organization;
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => {
                                ...
                                router.push(organizationIdFromUrl ? `${url}?organization=${organizationIdFromUrl}` : url);
                            }}>
                                <Eye className="mr-2 h-4 w-4" /> View Dashboard
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => {
                                const targetOrganizationId = organizationIdFromUrl || (typeof unit.organization === 'object' ? unit.organization?._id || unit.organization?.id : unit.organization);
                                const identifier = unit.slug || unit.id || unit._id;
                                ...
                                if (role === 'organization-owner' || role === 'organization-admin' || pathname?.includes('/organization')) {
                                     const targetUrl = organizationIdFromUrl ? `${organizationAdminUrl}?organization=${organizationIdFromUrl}` : organizationAdminUrl;
