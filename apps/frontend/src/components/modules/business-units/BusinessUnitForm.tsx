...
export function BusinessUnitForm({ slug }: BusinessUnitFormProps = {}) {
    ...
    const { data: rawOrganizations, isLoading: isLoadingOrganizations } = useGetAllOrganizationsQuery({});
    ...
    const allOrganizations = Array.isArray(rawOrganizations) ? rawOrganizations : (rawOrganizations?.data || rawOrganizations?.result || []);
    ...
    }, [isEditMode, organizationIdFromUrl, allOrganizations, isSuperAdmin, user, rawOrganizations, isLoadingOrganizations]);
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            ...
            organization: organizationIdFromUrl || (allOrganizations.length === 1 && !isSuperAdmin ? (allOrganizations[0]._id || allOrganizations[0].id) : ""),
        },
    });

    const selectedOrganizationId = form.watch("organization") || organizationIdFromUrl;
    const parentOrganization = allOrganizations.find((c: any) => c?._id === selectedOrganizationId || c.id === selectedOrganizationId);
    ...
    useEffect(() => {
        if (!isEditMode && organizationIdFromUrl && !form.getValues("organization")) {
    ...
    useEffect(() => {
        if (!isEditMode && !organizationIdFromUrl && allOrganizations.length === 1 && !form.getValues("organization")) {
            const singleOrganizationId = allOrganizations[0]._id || allOrganizations[0].id;
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        try {
            const finalOrganizationId = values.organization || selectedOrganizationId || organizationIdFromUrl;
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    */}
                                    {(organizationIdFromUrl || (!isSuperAdmin && allOrganizations.length === 1)) ? (
