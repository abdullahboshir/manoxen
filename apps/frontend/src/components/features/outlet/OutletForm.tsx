...
export function OutletForm({
  ...
  const pathOrgSlug = params.organization as string;
  const pathBUSlug = (params["business-unit"] || params.businessUnit) as string;
  ...
  useEffect(() => {
    ...
    if (preSelectedBusinessUnitId && currentVal !== preSelectedBusinessUnitId) {
      ...
      form.setValue("businessUnitId", preSelectedBusinessUnitId, {
        shouldValidate: true,
      });
      return;
    }
    ...
    if (currentBusinessUnit) {
      ...
      if (buId) {
        ...
        form.setValue("businessUnitId", buId, { shouldValidate: true });
        return;
      }
    }
    ...
    for (const slug of candidateSlugs) {
      ...
      if (isObjectId(slug)) {
        ...
        if (isBUId) {
          ...
          form.setValue("businessUnitId", slug, { shouldValidate: true });
          return;
        }
      }
      ...
      if (resultId) {
        form.setValue("businessUnitId", resultId, { shouldValidate: true });
        return;
      }
    }
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      const selectedBU = businessUnits.find(
        (bu: any) =>
          bu._id?.toString() === values.businessUnitId ||
          bu.id === values.businessUnitId,
      );
