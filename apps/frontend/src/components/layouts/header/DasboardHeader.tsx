...
export default function DashboardHeader({
  ...
  const { data: allBusinessUnits } = useGetBusinessUnitsQuery(
    (isSuperAdmin || isOrganizationOwner) && urlOrganizationId
      ? { organization: urlOrganizationId }
      : undefined,
    { skip: !(isSuperAdmin || isOrganizationOwner) },
  );

  const businessUnits = Array.isArray(allBusinessUnits)
    ? allBusinessUnits
  ...
  const handleNewSale = () => {
    if (businessUnitSlug) router.push(`/${businessUnitSlug}/sales/create`);
  };
