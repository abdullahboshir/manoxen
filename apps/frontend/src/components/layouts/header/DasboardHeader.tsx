...
export default function DashboardHeader({ onMenuClick }: DashboardHeaderProps = {}) {
  ...
  const { data: allBusinessUnits } = useGetBusinessUnitsQuery(
    (isSuperAdmin || isOrganizationOwner) && urlOrganizationId ? { organization: urlOrganizationId } : undefined,
    { skip: !(isSuperAdmin || isOrganizationOwner) }
  );
  ...
  const rawEffectiveOrganizationId = (isGlobalRoute && !urlOrganizationId && isSuperAdmin)
    ? null
    : (urlOrganizationId || derivedOrganizationId || (isOrganizationOwner ? (singleOrganizationId || localStorage.getItem('active-organization-id')) : null));
        ...
        <div className="flex h-16 items-center px-4 gap-4">
          <div className="flex items-center gap-2 lg:min-w-[200px]">
            ...
            {(isSuperAdmin || (organizations?.length > 1)) && (
              <OrganizationSwitcher currentOrganizationId={effectiveOrganizationId} companies={organizations} />
            )}
            {effectiveOrganizationId && (
              <BusinessUnitSwitcher currentBusinessUnit={businessUnitSlug || 'all'} effectiveOrganizationId={effectiveOrganizationId} currentRole={role} availableUnits={availableUnits} />
            )}
            {businessUnitSlug && (
              <OutletSwitcher currentBusinessUnitSlug={businessUnitSlug} effectiveOrganizationId={effectiveOrganizationId} activeUnit={activeUnit || activeUnitFromUrl} />
            )}
          </div>
