...
This document outlines the "industrial standard" multi-tenancy and context-aware isolation layer implemented in the **Manoxen POS** ecosystem. The system is designed for **Zero Trust**, ensuring that data isolation is enforced at the architectural level (Mongoose Plugins + Context Service) rather than relying on developer discipline.
...
graph TD
    ...
    ORG["🏢 ORGANIZATION (Merchant/Organization)"]
...
- **Injects Filters:** Prepends `organizationId`, `businessUnitId`, or `outletId` to all `find`, `aggregate`, and `update` queries.
...
> [!IMPORTANT]
>
> - **No Upward Access:** An OUTLET level user can _never_ see data belonging to the parent BUSINESS UNIT or siblings.
