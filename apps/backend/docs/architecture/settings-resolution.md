...
**Scope:** All business units under a organization  
**File:** `platform/organization/organization/settings/`
...
function resolveSetting(key: string, context: Context) {
  ...
  if (context.organizationId) {
    const value = await OrganizationSettings.get(context.organizationId, key);
    if (value !== undefined) return value;
  }
...
const currency = await resolveSettings("currency", {
  ...
  organizationId: "organization-789",
});
