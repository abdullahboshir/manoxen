...
const requireModule = (moduleName: string) => {
    return catchAsync(async (req: Request, _res: Response, next: NextFunction) => {
        ...
        // Strategy: 
        // A. If header `x-organization-id` or `x-organization-id` is present, check that organization.
        ...
        const contextOrganizationId = (req.headers['x-organization-id'] || req.headers['x-organization-id']) as string;
        ...
        const organizationModules = user.organizationModules || user.organizationModules || {};
