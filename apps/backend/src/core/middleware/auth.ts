...
import { permissionService } from "#app/modules/iam/permission/permission.service"
import { BusinessUnit } from "#app/modules/platform/index"
...
const auth = (...requiredRoles: string[]) => {
  return catchAsync(async (req: Request, _res: Response, next: NextFunction) => {
    ...
    let organizationHeaderId = (req.headers['x-organization-id'] || req.headers['x-organization-id']) as string;
    ...
    if (!activeOrganizationId && businessUnitId && Array.isArray(isUserExists.businessAccess)) {
      ...
      if (buFound && (buFound.organization || buFound.organization)) {
        activeOrganizationId = (buFound.organization?._id || buFound.organization?.id || buFound.organization?._id || buFound.organization?.id || buFound.organization || buFound.organization).toString();
      }
    }
    ...
    if (outletId && outletId !== 'new' && mongoose.Types.ObjectId.isValid(outletId) && (!activeOrganizationId || !businessUnitId)) {
      ...
      if (outlet) {
        if (outlet.businessUnit) {
          ...
          if (!activeOrganizationId && (bu.organization || bu.organization)) {
            activeOrganizationId = (bu.organization || bu.organization).toString();
          }
        }
      }
    ...
    if (isUserExists.isSuperAdmin) {
      ...
      if (Array.isArray(isUserExists.businessAccess)) {
        isUserExists.businessAccess.forEach((a: any) => {
          ...
          if ((a.scope === 'ORGANIZATION' || a.scope === 'ORGANIZATION') && activeOrganizationId && 
    ...
    if (requiredRoles.length > 0) {
      if (!isUserExists.isSuperAdmin) {
        ...
        const isOrganizationOwner = effectiveRoleNames.includes('organization-owner') || effectiveRoleNames.includes('organization-owner');
