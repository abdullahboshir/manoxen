import type { IPermissionContext } from "@manoxen/iam-core";
...
export const authorize = (resource: string, action: string) => {
  return catchAsync(async (req: any, _res: any, next: any) => {
    try {
      ...
      const userWithRoles = await User.findById(user.userId || user.id)
        ...
        .populate({
          ...
          populate: [
            ...
            {
              path: 'businessUnit',
              select: 'name slug id activeModules organization organization',
              populate: [
      ...
      // --------------------------------------------------------------------------
      // ðŸ›¡ï¸ MODULE LEVEL GUARD (Tiered: System > Organization > BU)
      // --------------------------------------------------------------------------
      ...
      if (requiredModule) {
        ...
        if (targetBuParam) {
          ...
          if (targetAccess?.businessUnit) {
            ...
            const org = targetAccess.businessUnit.organization || targetAccess.businessUnit.organization;
