import { AuditLog, type IAuditLog } from "#app/modules/platform/system/audit-log/audit-log.model";
    ...
    static async log(payload: {
        ...
        try {
            ...
            if (!payload.actor.userId) {
                console.warn('âš ï¸ [AuditService] Missing userId for audit log', payload);
                return;
            }
            ...
            AuditLog.create(logEntry).catch(err => {
                console.error('âŒ [AuditService] Failed to persist log:', err.message);
            });

        } catch (error) {
            // Prevent audit failure from crashing the app
            console.error('âŒ [AuditService] Critical Error:', error);
        }
    }
}
