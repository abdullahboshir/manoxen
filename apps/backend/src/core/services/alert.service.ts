...
import { DEFAULT_LEGAL_GOVERNANCE } from "#app/modules/platform/organization/shared/common.defaults";
...
export class AlertService {
    ...
    static async sendCriticalAlert(title: string, error: any, context?: Record<string, any>) {
        if (appConfig.NODE_ENV === 'development') {
            console.log(`ðŸ”” [AlertService] Skipped email in Development: ${title}`);
            return;
        }
        ...
        const html = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px;">
                <div style="background-color: #DC2626; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">
                    <h1>ðŸš¨ CRITICAL SYSTEM ALERT</h1>
                </div>
...
${stackTrace}
                    ...
                    <p style="font-size: 12px; color: #666; text-align: center;">
                        This is an automated alert from <strong>Manoxen Systems</strong>.
        ...
        await MailService.sendEmail(
            this.ownerEmail,
            `ðŸš¨ CRITICAL: ${title} - ${appConfig.NODE_ENV.toUpperCase()}`,
            html
        );
    }
    ...
    static async sendWarning(title: string, message: string) {
        ...
        console.warn(`âš ï¸ [AlertService] WARNING: ${title} - ${message}`);
    }
}
