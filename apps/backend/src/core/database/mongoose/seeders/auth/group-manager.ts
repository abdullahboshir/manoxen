import { PermissionGroup } from "#app/modules/iam/permission-group/permission-group.model";
import { Permission } from "#app/modules/iam/permission/permission.model";
import { PermissionActionType, PermissionSourceObj } from "@manoxen/iam-core";
import { getModuleByResource } from "@manoxen/iam-core";
import { getScopeForResource } from "./data/scope-map";
import mongoose, { Types } from "mongoose";
import { SYSTEM_USER_ID } from "./data/constants";
...
export async function syncPermissions(session?: mongoose.ClientSession): Promise<void> {
    ...
    if (permissionOps.length > 0) {
        try {
            ...
            console.log(`âœ… ${permissionOps.length} Permissions synced.`);
            ...
            if (result.hasWriteErrors()) {
                console.error("âŒ BulkWrite has errors:", result.getWriteErrors());
            }
            ...
            console.error("âŒ Permission sync failed:", error);
            throw error;
        }
    } else {
        console.log("â„¹ï¸ No permissions to sync.");
    }
}
...
export async function syncResourceGroups(session?: mongoose.ClientSession): Promise<Record<string, mongoose.Types.ObjectId>> {
    ...
    for (const [resource, permIds] of Object.entries(permissionsByResource)) {
        ...
        if (!group) {
            ...
            if (nativeGroup) {
                console.error(`ðŸš¨ CRITICAL: Mongoose missed existing group '${name}' but Native found it! Possible Schema/Plugin issue.`);
        ...
        if (!group) {
            try {
                ...
                if (e.code === 11000) {
                    console.log(`âš ï¸ Race condition avoided for ${name}, fetching again.`);
                    group = await PermissionGroup.findOne({ name }).session(session ?? null);
                } else {
                    console.log(`âš ï¸ Group creation failed for ${name}, might exist.`, e.message);
                    continue;
                }
            }
        } else {
    ...
    try {
        ...
        if (!fullAccessGroup) {
            try {
                ...
                if (e.code === 11000) {
                    console.log(`âš ï¸ Race condition avoided for system.full_access.`);
        ...
        console.error("âŒ Failed to sync full access group:", e);
    }
...
export function auditResourceGroups(resourceGroupsMap: Record<string, Types.ObjectId>) {
    ...
    if (missingGroups.length > 0) {
        console.warn(`âš ï¸  CRITICAL: Missing Permission Groups for resources in PermissionSourceObj:`, missingGroups);
    } else {
        console.log("âœ…  All resources in PermissionSourceObj have valid PermissionGroups.");
    }
}
