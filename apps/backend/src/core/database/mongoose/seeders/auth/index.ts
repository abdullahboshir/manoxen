...
import { auditResourceGroups, syncPermissions, syncResourceGroups } from "./group-manager";
import { getPlatformRoleConfigs } from "./platform.seeder";
import { getOrganizationRoleConfigs } from "./organization.seeder";
import { getBusinessRoleConfigs } from "./business.seeder";
import { getOutletRoleConfigs } from "./outlet.seeder";
import { DEFAULT_LIMITS, SYSTEM_USER_ID } from "./data/constants";
import { Role } from "#app/modules/iam/index";
import { seedSuperAdminUser } from "./super-admin.user";


/**
 * ðŸ›¡ï¸ Main Authorization Seeder Orchestrator (Hybrid Split)
...
export async function runRolePermissionSeeder({ clean = false, session }: { clean?: boolean, session?: mongoose.ClientSession } = {}) {
    console.log("DEBUG: runRolePermissionSeeder ACTIVE SOURCE READ");
    console.log("--- ðŸ—ï¸ Modular Authorization Seeder Started ---");
    ...
    if (clean) {
        console.log("ðŸ§¹ Cleaning existing authorization data...");
    ...
    console.log("ðŸ› ï¸  Step 1: Syncing Permissions & Resource Groups...");
    ...
    console.log(`ðŸ“¡ Step 2: Syncing ${roleConfigs.length} Roles...`);
    try {
        ...
        console.error("âŒ Failed to seed roles atomically:", error);
        throw error;
    }

    // 5. Final Step: Ensure Super Admin User exists
    console.log("ðŸ‘¤ Step 3: Syncing Super Admin User...");
