import { Role, User, USER_ROLE, USER_STATUS } from "#app/modules/iam/index";
 ...
 * âš ï¸ This seeder is IDEMPOTENT and relies on the SUPER_ADMIN role being created first.
 */
export async function seedSuperAdminUser(session?: mongoose.ClientSession) {
    ...
    try {
        ...
        const query = User.findOneAndUpdate(
            { id: SUPER_ADMIN_ID },
            {
                $set: {
                    ...
                    nameBangla: "à¦¸à§à¦ªà¦¾à¦° à¦…à§à¦¯à¦¾à¦¡à¦®à¦¿à¦¨",
                    ...
                    descriptionBangla: "à¦¸à¦®à§à¦ªà§‚à¦°à§à¦£ à¦¸à¦¿à¦¸à§à¦Ÿà§‡à¦®à§‡ à¦…à§à¦¯à¦¾à¦•à§à¦¸à§‡à¦¸",
        ...
        // without organization filtering restrictions.
        (query as any)._bypassContext = true;

        await query.exec();

        console.log("âœ… Super Admin user synced (Atomic + Context Bypass).");

    } catch (error: any) {
        console.error("âŒ Failed to seed Super Admin:", error.message);
        throw error;
    }
}
