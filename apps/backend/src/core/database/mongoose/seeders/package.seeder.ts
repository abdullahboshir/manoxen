...
import { type IPackage } from "#app/modules/platform/package/package.interface";
...
export const seedPackages = async () => {
    console.log("ðŸŒ± [Zenith Sync] Synchronizing Package Data...".blue.bold);
    ...
    for (const pkg of packages) {
        try {
            ...
            if (result) {
                ...
                console.log(`   âœ… Synced: ${displayName.padEnd(20)} [Slug: ${(pkg.slug || 'unknown').cyan}]`);
            }
        } catch (err: any) {
            console.error(`   âŒ Sync Failed [${pkg.slug}]:`.red, err.message);
        }
    }

    console.log("âœ¨ Package Synchronization Completed Successfully".green.bold);
};
