...
import { Leave, type ILeave } from "../../infrastructure/persistence/mongoose/leave.model";

import httpStatus from "http-status";
import mongoose, { Types } from "mongoose";


// Local interface for type safety without dependency
interface IBusinessUnitLookup {
    _id: Types.ObjectId;
    slug: string;
}
...
const createLeave = async (userId: string, payload: Partial<ILeave>) => {
    ...
    if (payload.businessUnit) {
        ...
        if (!isObjectId) {
            // Runtime model access to avoid circular dependency/rootDir issues
            const BusinessUnit = (mongoose.models['BusinessUnit'] || mongoose.model<IBusinessUnitLookup>("BusinessUnit")) as mongoose.Model<IBusinessUnitLookup>;
            const bu = await BusinessUnit.findOne({ slug: payload.businessUnit as any });
            if (bu) payload.businessUnit = bu._id as any;
        }
    }
