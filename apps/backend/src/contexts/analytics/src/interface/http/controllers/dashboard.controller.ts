...
// Get aggregated dashboard stats for super admin or organization owner
export const getDashboardStatsController = catchAsync(async (req, res) => {
    ...
    if (!isSuperAdmin) {
        const objectOrganizationIds = organizationIds.map((id: string) => new mongoose.Types.ObjectId(id));
        buFilter.organization = { $in: objectOrganizationIds };
        commerceFilter.organization = { $in: objectOrganizationIds };
        organizationFilter._id = { $in: objectOrganizationIds };

        const uniqueUserIds = await UserBusinessAccess.distinct("user", {
            organization: { $in: objectOrganizationIds },
            status: "ACTIVE"
        });
    ...
    // 1. Counts
    const totalOrganizations = await Organization.countDocuments(organizationFilter);
    const activeOrganizations = await Organization.countDocuments({ ...organizationFilter, isActive: true });
    ...
    const stats = {
        ...
        activeOrganizations: activeOrganizations, // Legacy field support if needed
