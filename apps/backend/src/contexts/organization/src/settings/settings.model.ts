...
const organizationSettingsSchema = new Schema<IOrganizationSettings, IOrganizationSettingsModel>({
...
// Static method to get or create settings for a specific organization
organizationSettingsSchema.statics['getSettings'] = async function (organizationId: string, session?: any) {
    let settings = await this.findOne({ organization: organizationId }).session(session || null);
    if (!settings) {
        ...
        return created[0] as IOrganizationSettings;
    }
    return settings as IOrganizationSettings;
};

export const OrganizationSettings = model<IOrganizationSettings, IOrganizationSettingsModel>('OrganizationSettings', organizationSettingsSchema);
