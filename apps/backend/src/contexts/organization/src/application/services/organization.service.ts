...
import { OrganizationRepository } from "../../infrastructure/persistence/mongoose/organization.repository";
...
import { UserBusinessAccess } from "@manoxen/iam";
import { createOrganizationOwnerService } from "@manoxen/iam";
...
export class OrganizationService {
  async createOrganization(
    ...
    try {
      ...
      if (!email || !name || !phone) {
        throw new Error(
          "Missing required organization contact details for owner creation",
        );
      }
      ...
      const trialPackage =
        (await Package.findOne({ slug: "trial" }).session(session)) ||
        (await Package.findOne({ isActive: true }).session(session));
          ...
          [
            {
              ...
              expiresAt: new Date(
                Date.now() +
                  (trialPackage.trialPeriodDays ?? 14) * 24 * 60 * 60 * 1000,
  ...
  async getAllOrganizations(user: any): Promise<IOrganizationDocument[]> {
    ...
    if (
      ...
      if (authorizedOrganizationIds.length > 0) {
        organizations = await organizationRepository.findAll({
          _id: { $in: authorizedOrganizationIds },
        });
      } else {
        ...
        const organizationIds = [
          ...new Set(
            businessAccess
              .map((access: any) => access.organization?.toString())
              .filter(Boolean),
          ),
        ];
        organizations = await organizationRepository.findAll({
    ...
    if (organizations.length > 0) {
      const organizationIds = organizations.map((c: any) => c._id);
      ...
      const buIds = businessUnits.map((b: any) => b._id);
      const buOrganizationMap = businessUnits.reduce(
        (acc: Record<string, string>, bu: any) => {
      ...
      licenses.forEach((license: any) => {
        ...
        if (organizationId) {
          ...
          if (
            ...
            (organizationLicenseMap[organizationId].status !== "active" &&
      ...
      return organizations.map((organization: any) => {
        const organizationObj = organization.toObject
          ...
          : organization;
        const subscription = organizationLicenseMap[
          organizationObj._id.toString()
        ] || {
          planName: "No Plan",
          status: "inactive",
          expiresAt: null,
  ...
  async getOrganizationDashboardStats(organizationId: string) {
    try {
      const { Order } = await import("@manoxen/sales");
      const { User } = await import("@manoxen/iam");
      const { Purchase } = await import("@manoxen/supply");
      const { Expense } = await import("@manoxen/pos");
      ...
      const buIds = businessUnits.map((bu: any) => bu._id);
      ...
      console.error(
        "Failed to get organization dashboard stats",
        error.message,
      );
  ...
  async updateTenantConfig(
    ...
    if (organization) {
      // Invalidate caches
      const { TenantService } = await import("@manoxen/tenant-strategy");
      const { TenantStorageService } = await import("@manoxen/infra-common");
      ...
      if (organization.tenantConfig?.customDomain) {
        await TenantService.invalidateCache(
          organization.tenantConfig.customDomain,
        );
      }
