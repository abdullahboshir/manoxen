...
import { QueueService } from "#domain/system/index.js";
import { QUEUE_NAMES } from "#domain/system/index.js";
...
import { BusinessUnit, Organization } from "#domain/organization/index.js";
import type { ILicense } from "#domain/organization/license/license.interface.js";
import { License } from "#domain/organization/license/license.model.js";
...
const syncModulesToTarget = async (license: ILicense) => {
    try {
        ...
        if (!bu) {
            console.error("âš ï¸ Sync Aborted: BusinessUnit not found for license:", license.clientId);
            return;
        }
        ...
        if (bu.organization) {
            await Organization.findByIdAndUpdate(bu.organization, { $set: { activeModules } });
            console.log(`âœ… Modules synced to Organization: ${bu.organization}`);
        }

    } catch (error) {
        console.error("âŒ Failed to sync modules to target:", error);
    }
};
...
const resolveBusinessUnitId = async (rawId: any): Promise<{ id: Types.ObjectId, name: string }> => {
    console.log("ï¿½ Resolving Business Unit ID for input:", JSON.stringify(rawId));

    // Catch null, undefined, or empty strings explicitly
    if (rawId === null || rawId === undefined || rawId === '') {
        console.error("âŒ Resolution Failed: Input is null, undefined, or empty string.");
    ...
    if (typeof rawId === 'object') {
        target = rawId._id || rawId.id || rawId.value || rawId.clientId || rawId.organizationId || rawId;
    }

    // Re-verify after extraction
    if (!target || target === '' || typeof target === 'object') {
        console.error("âŒ Failed to resolve primitive ID from complex input:", rawId);
    ...
    console.log("ðŸ“ Extracted ID String:", targetStr);
    ...
    if (isObjectId) {
        ...
        if (!bu) {
            console.log("ðŸ¢ ID is not a BusinessUnit, checking as Organization ID...");
            // Check if it's a Organization ID, get primary BU
            bu = await BusinessUnit.findOne({ organization: buId }).sort({ createdAt: 1 });
        }
        if (bu) {
            console.log("âœ… Resolved to BusinessUnit:", bu.name, `(${bu._id})`);
            return { id: bu._id as Types.ObjectId, name: bu.name };
        }
    } else {
        // Handle Slugs
        console.log("ðŸš ID is not an ObjectId, checking as Slug:", targetStr);
        const bu = await BusinessUnit.findOne({ slug: targetStr });
        if (bu) {
            console.log("âœ… Resolved Slug to BusinessUnit:", bu.name, `(${bu._id})`);
            return { id: bu._id as Types.ObjectId, name: bu.name };
        }
    }

    console.error("âŒ Resolution Failed: Target not found for identity:", targetStr);
    throw new AppError(httpStatus.NOT_FOUND, `Target Business Unit or Organization not found for identity: ${targetStr}`);
};

/**
 * ðŸ’° Unified Pricing Logic (Absolute Zenith Standard)
...
const createLicense = async (payload: Partial<ILicense>) => {
    try {
        // ðŸ› ï¸ Robust ID Resolution (Ensures BusinessUnit ObjectId)
        const rawId = payload.clientId || (payload as any).organizationId || (payload as any).organization;
        ...
        // ðŸ’° Dynamic Pricing Calculation (State-Aware Merged State)
        ...
        console.log("ðŸ’Ž Creating License | clientId:", payload.clientId, "| packageId:", payload.packageId);
        ...
        // ðŸ”„ Sync Modules Immediately (Nirvhul Implementation)
        await syncModulesToTarget(result);

        console.log("âœ… License created and modules synced.");
        return result;
    } catch (error: any) {
        console.error("âŒ License Creation Failed:", error);
...
const updateLicense = async (licenseId: string, payload: Partial<ILicense>) => {
    try {
        console.log(`â™»ï¸  Attempting update for License: ${licenseId}`);
        // ðŸ›¡ï¸ Prevent key update safely & sanitize
        ...
        // ðŸ› ï¸ Robust ID Resolution for Updates
        ...
        // ðŸ’° Pricing Recalculation (State-Aware Merged State)
        ...
        if (payload.customModules || payload.overriddenLimits || payload.packageId) {
            console.log("ðŸ’° Configuration change detected, recalculating merged price...");
        ...
        console.log("ðŸš€ Saving update to database with clientId:", payload.clientId);
        ...
        console.error("âŒ License Update Denied:", error);
        if (error.name === 'ValidationError') {
            const messages = Object.values(error.errors).map((err: any) => err.message);
...
const handleLicenseExpirations = async (): Promise<{ updated: number, emailed: number }> => {
    ...
    for (const license of expiredLicenses) {
        ...
        const bu = await BusinessUnit.findById(license.clientId).populate('organization');
        const contactEmail = bu?.contact?.email || (bu?.organization as any)?.contact?.email;
