...
const createBusinessUnitBaseSchema = z.object({
  ...
  slug: z.string()
    ...
    .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, "Slug can only contain lowercase letters, numbers, and hyphens").optional(),
  ...
  operationalModel: z.string().optional(),
  ...
  activeModules: z.object({
    ...
    commerce: moduleStatusSchema.optional(),
...
export const createBusinessUnitValidationSchema = z.object({
  body: createBusinessUnitBaseSchema
});

export const updateBusinessUnitValidationSchema = z.object({
  body: createBusinessUnitBaseSchema.partial()
});
