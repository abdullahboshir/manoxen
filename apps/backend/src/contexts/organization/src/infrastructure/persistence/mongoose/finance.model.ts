...
import type { IBusinessUnitFinanceDocument, IBusinessUnitFinanceModel } from "./finance.interface";
...
businessUnitFinanceSchema.virtual('netProfit').get(function () {
  return (this as any).grossProfit - this.expenses.totalExpenses - this.commissions.totalCommissions;
});

businessUnitFinanceSchema.virtual('profitMargin').get(function () {
  return this.revenue.totalRevenue > 0 ? ((this as any).netProfit / this.revenue.totalRevenue) * 100 : 0;
});
