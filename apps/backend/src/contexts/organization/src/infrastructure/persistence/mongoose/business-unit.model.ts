...
businessUnitCoreSchema.statics.searchBusinessUnits = function (
  ...
  if (filters.domain) searchFilter.domain = filters.domain;
...
businessUnitCoreSchema.statics.findBusinessUnitsNeedingAttention =
  function (): Promise<IBusinessUnitCoreDocument[]> {
    return this.find({
      status: "published",
      $or: [
...
export const BusinessUnit = model<
  IBusinessUnitCoreDocument,
  IBusinessUnitCoreModel
>("BusinessUnit", businessUnitCoreSchema);
