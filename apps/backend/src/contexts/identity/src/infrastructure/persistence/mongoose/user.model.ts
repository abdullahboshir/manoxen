...
export interface IUserModel extends Model<IUserDocument> {
  ...
  isJWTIssuedBeforePasswordChanged(passwordChangedAtTime: Date, jwtIssuedTime: number): boolean;
}
...
const UserSchema = new Schema<IUserDocument, IUserModel>({
  ...
  globalRoles: { type: [Schema.Types.ObjectId as any], ref: "Role", default: [] },
  region: { type: String },
  ...
  organization: { type: Schema.Types.ObjectId as any, ref: 'Organization', index: true },
  businessAccess: [{ type: Schema.Types.ObjectId, ref: 'UserBusinessAccess' }],
...
UserSchema.statics["isPasswordMatched"] = async function (plainPass: string, hashedPass: string): Promise<boolean> {
...
UserSchema.statics["isUserExists"] = async function (email: string): Promise<IUserDocument> {
  const user = await this.findOne({ email, isDeleted: false, isActive: true })
    .populate([
      {
        path: 'globalRoles',
        populate: [
          { path: 'permissions', model: 'Permission', select: 'resource action scope effect conditions resolver attributes' },
          {
      ...
      {
        path: 'businessAccess',
        select: 'role scope organization businessUnit outlet status isPrimary dataScopeOverride',
        populate: [
          {
            path: 'role',
