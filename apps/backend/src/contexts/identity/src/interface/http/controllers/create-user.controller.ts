...
export const createUserController = catchAsync(async (req: any, res) => {
  ...
  if (
    ...
    roleId =
      typeof firstRole === "string" ? firstRole : firstRole._id || firstRole.id;
  }
  ...
  if (roleName === "CUSTOMER") {
    ...
    result = await createCustomerService(
      customerPayload,
      userData.password,
      file,
    );
  } else if (
    [USER_ROLE.SUPER_ADMIN, USER_ROLE.GUEST, USER_ROLE.VENDOR]
      .map((r) => r.toUpperCase())
      .includes(roleName)
  ) {
    ...
    try {
      ...
      if (userData.businessUnits && Array.isArray(userData.businessUnits)) {
        userData.businessUnits = await resolveBusinessUnitIds(
          userData.businessUnits,
        );
      }
    ...
    try {
      ...
      if (
        ...
        // 2. Resolve organization (might be slug or ID)
        let resolvedOrgId = userData.organization;
        ...
        // 3. Create requested accesses with resolved IDs
        const accessPayloads = await Promise.all(
          userData.permissions.map(async (p: any) => {
            let orgId = p.organization || resolvedOrgId;
            let buId = p.businessUnit;

            // Resolve organization from slug if needed
            if (orgId && !mongoose.Types.ObjectId.isValid(orgId)) {
              const orgDoc = await Organization.findOne({ slug: orgId }).select(
                "_id",
              );
              orgId = orgDoc?._id || null;
            }

            // Resolve businessUnit from slug if needed
            if (buId && !mongoose.Types.ObjectId.isValid(buId)) {
              const buDoc = await BusinessUnit.findOne({ slug: buId }).select(
                "_id organization",
              );
              if (buDoc) {
                buId = buDoc._id;
                // Backfill organization from BU if not provided
                if (!orgId && buDoc.organization) {
                  orgId = buDoc.organization;
                }
              }
            }
            ...
            if (!orgId && buId) {
              const buDoc =
                await BusinessUnit.findById(buId).select("organization");
            ...
            return {
              ...
              scope:
                p.scope || (p.outlet ? "OUTLET" : buId ? "BUSINESS" : "GLOBAL"),
              status: "ACTIVE",
        ...
        console.log(
          `>> Updated ${accessPayloads.length} permissions for user ${userId}`,
        );
      }
    } catch (error) {
