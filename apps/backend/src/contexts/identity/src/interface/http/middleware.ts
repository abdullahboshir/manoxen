...
import { verifyToken } from "@manoxen/iam-core";
...
import { permissionService, User, UserBusinessAccess } from "@manoxen/iam";
import { BusinessUnit } from "@manoxen/organization/index";
import AppError from "packages/platform-core/src/errors/app-error";
import { appConfig } from "packages/platform-core/src/config";
...
const auth = (...requiredRoles: string[]) => {
  return catchAsync(
    async (req: Request, _res: Response, next: NextFunction) => {
      ...
      const decoded = verifyToken(token, appConfig.jwt_access_secret as string);
      ...
      if (
        ...
        isUserExists.businessAccess.length === 0
      ) {
      ...
      if (
        ...
        throw new AppError(
          status.UNAUTHORIZED,
          "You are not authorized - password changed",
        );
      }
      ...
      let organizationHeaderId = (req.headers["x-organization-id"] ||
      ...
      if (
        ...
        businessUnitId &&
        Array.isArray(isUserExists.businessAccess)
      ) {
        ...
        if (buFound && buFound.organization) {
          activeOrganizationId = (
            buFound.organization._id ||
            buFound.organization.id ||
            buFound.organization
          ).toString();
        }
      }
      ...
      if (!activeOrganizationId && businessUnitId) {
        ...
        if (bu && bu.organization) {
          activeOrganizationId = bu.organization.toString();
        }
      }
      ...
      if (
        ...
        (!activeOrganizationId || !businessUnitId)
      ) {
        const OutletModel =
          mongoose.models["Outlet"] || mongoose.model("Outlet");
        // Use any for safe populate access without direct type import
        const outlet = (await OutletModel.findById(outletId)
          .populate(["businessUnit", "organization"])
          .lean()) as any;
        ...
        if (outlet) {
          if (outlet.businessUnit) {
            ...
            if (!activeOrganizationId && bu.organization) {
              activeOrganizationId = bu.organization.toString();
            }
          }
        }
      ...
      if (!isUserExists.isSuperAdmin) {
        ...
        if (activeOrganizationId) {
          const hasOrganizationAccess = authorizedAccess.some(
            (a: any) =>
          ...
          if (!hasOrganizationAccess) {
            ...
            throw new AppError(
              status.FORBIDDEN,
              `Access Denied: You do not have access to organization ${activeOrganizationId}`,
            );
          }
        }

        if (businessUnitId) {
          const hasBUAccess = authorizedAccess.some(
            (a: any) =>
              a.businessUnit?._id?.toString() === businessUnitId ||
              a.businessUnit?.slug === businessUnitId ||
              a.businessUnit?.toString() === businessUnitId || // Fix: Handle String ID
              a.businessUnit?.id === businessUnitId ||
              (a.scope === "ORGANIZATION" &&
                activeOrganizationId &&
                (a.organization?._id?.toString() === activeOrganizationId ||
                  a.organization?.toString() === activeOrganizationId)) ||
              a.scope === "GLOBAL",
          );
          if (!hasBUAccess) {
            ContextService.logSecurityAlert({
        ...
        if (outletId) {
          const hasOutletAccess = authorizedAccess.some(
            (a: any) =>
      ...
      if (Array.isArray(isUserExists.globalRoles)) {
        isUserExists.globalRoles.forEach((r: any) =>
          effectiveRoleNames.push(r.name),
        );
      }
      ...
      if (isUserExists.isSuperAdmin) {
        ...
        if (Array.isArray(isUserExists.businessAccess)) {
          isUserExists.businessAccess.forEach((a: any) => {
            // Normalize status
            const statusMatch =
              !a.status || a.status.toUpperCase() === "ACTIVE";
            ...
            // ORGANIZATION or ORGANIZATION scope applies if it matches the active organization
            if (
              (a.scope === "ORGANIZATION" || a.scope === "ORGANIZATION") &&
              activeOrganizationId &&
              (a.organization?._id?.toString() === activeOrganizationId ||
                a.organization?.id === activeOrganizationId ||
                a.organization?.toString() === activeOrganizationId ||
                a.organization?._id?.toString() === activeOrganizationId ||
                a.organization?.id === activeOrganizationId ||
                a.organization?.toString() === activeOrganizationId)
      ...
      const authContext = await permissionService.getAuthorizationContext(
        isUserExists as any,
        businessUnitId || activeOrganizationId
          ? {
              businessUnitId,
      ...
      if (requiredRoles.length > 0) {
        if (!isUserExists.isSuperAdmin) {
          ...
          const isOrganizationOwner =
            effectiveRoleNames.includes("organization-owner") ||
            effectiveRoleNames.includes("organization-owner");

          const hasRequiredRole =
            isOrganizationOwner ||
            requiredRoles.some((reqRole) =>
              effectiveRoleNames.includes(reqRole),
            );
          if (!hasRequiredRole) {
            throw new AppError(
              status.FORBIDDEN,
              `Access Denied. You do not have permission for this context.`,
            );
          }
        }
      }
      ...
      if (Array.isArray(isUserExists.businessAccess)) {
        isUserExists.businessAccess.forEach((a: any) => {
          if (a.businessUnit)
            uniqueBusinessUnits.set(
              a.businessUnit._id.toString(),
              a.businessUnit,
            );
          ...
          if (a.outlet)
            uniqueOutlets.set(a.outlet._id.toString(), a.outlet._id.toString()); // Store outlet ID
        });
      }
      ...
      req.user = {
        ...
        ...(isUserExists.branches !== undefined && {
          branches: isUserExists.branches,
        }),
        ...(isUserExists.vendorId !== undefined && {
          vendorId: isUserExists.vendorId,
        }),
        ...(isUserExists.region !== undefined && {
          region: isUserExists.region,
        }),
        iat,
      } as JwtPayload;
      ...
      if (!isUserExists.isSuperAdmin) {
        ...
        if (businessUnitId) {
          ...
          const buObj = Array.from(uniqueBusinessUnits.values()).find(
            (b: any) =>
          ...
          if (buObj) activeDomain = buObj.domain;
            ...
            const fetchedBu = await BusinessUnit.findById(businessUnitId)
              .select("domain")
              .lean();
...
export const adminAuthMiddleware = auth("super-admin", "admin");
