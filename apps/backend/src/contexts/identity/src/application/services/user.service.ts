...
// Re-export from Contacts module to maintain legacy compatibility (CAUTION: violating DDD)
...
export const createCustomerService = async (
  ...
  try {
    ...
    const newCustomer: any = await CustomerModel.create([customerPayload], {
      session,
    });
    ...
    const result = await CustomerModel.findById(newCustomer[0]._id).populate({
      path: "user",
      select: "-password",
      populate: {
        path: "roles",
  ...
  try {
    ...
    try {
      ...
      await MailService.sendEmail(
        email,
        "Manoxen - Staff Account Invitation",
        `
            <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px;">
                ...
                <p>You have been added as a staff member to <strong>Manoxen</strong>.</p>
...
export const createOrganizationOwnerService = async (
  ...
  if (isUserExists) {
    ...
    if (isUserExists.status === USER_STATUS.PENDING) {
      ...
      try {
        ...
        await MailService.sendEmail(
          ...
          "Welcome to Manoxen - Set up your account",
          `
            <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px;">
    ...
    try {
      await MailService.sendEmail(
        organizationData.contactEmail,
        "New Organization Added - Manoxen",
        `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px;">
