...
import { PhysicalPropertiesSchema } from "../../product-shared.model";
import type { IProductVariantDocument } from "./product-variant.interface";


const productVariantSchema = new Schema<IProductVariantDocument>({
  product: { type: Schema.Types.ObjectId, ref: 'Product', required: true, unique: true } as any,
...
productVariantSchema.methods['findVariantByAttributes'] = function (attributes: Record<string, string>) {
  return (this as any)['variants'].find((variant: any) => {
...
productVariantSchema.methods['getAvailableVariants'] = function () {
  return (this as any)['variants'].filter((variant: any) =>
...
productVariantSchema.methods['getDefaultVariant'] = function () {
  return (this as any)['variants'].find((variant: any) => variant.isDefault) || (this as any)['variants'][0];
};
...
productVariantSchema.pre('save', function (next) {
  ...
  if ((this as any)['variants'].filter((v: any) => v.isDefault).length > 1) {
    const defaultVariants = (this as any)['variants'].filter((v: any) => v.isDefault);
