...
const createOutlet = async (payload: any, user?: any): Promise<any> => {
    ...
    try {
        // ðŸ›¡ï¸ Hierarchy Validation: Ensure Outlet modules are a subset of BU modules
        ...
        await OutletSettings.getSettings(result?._id as any, session);
...
const getAllOutlets = async (businessUnitId: string, user?: any): Promise<any> => {
    // ðŸ›¡ï¸ Data isolation is now centrally handled by queryContext + QueryBuilder
...
const updateOutlet = async (id: string, payload: Partial<IOutlet>): Promise<IOutlet | null> => {
    ...
    // ðŸ›¡ï¸ Hierarchy Validation on Update
    if (payload.activeModules) {
...
/**
 * ðŸ›¡ï¸ Validate that child modules are a subset of parent modules
 */
