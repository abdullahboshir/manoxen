...
import { Package } from "#app/modules/platform/package/package.model";
import { License } from "#app/modules/platform/license/license.model";
...
import { OrganizationSettings } from "./settings/settings.model";
import { UserBusinessAccess } from "#app/modules/iam/user-business-access/user-business-access.model";



import { createOrganizationOwnerService } from "#app/modules/iam/user/user.service";
...
export class OrganizationService {
    ...
    async getAllOrganizations(user: any): Promise<IOrganizationDocument[]> {
        ...
        if (organizations.length > 0) {
            ...
            const buOrganizationMap = businessUnits.reduce((acc: Record<string, string>, bu: any) => {
    ...
    async getOrganizationDashboardStats(organizationId: string) {
        try {
            const { Order } = await import("@manoxen/sales/sales/order/order.model");
            const { User } = await import("#app/modules/iam/user/user.model");
            const { Purchase } = await import("@manoxen/erp/purchase/purchase.model");
            const { Expense } = await import("@manoxen/pos/cash/expense/expense.model");
            ...
            const matchStage = { businessUnit: { $in: buIds.map((id: any) => new mongoose.Types.ObjectId(id.toString())) } };
    ...
    async updateTenantConfig(id: string, config: any): Promise<IOrganizationDocument | null> {
        ...
        if (organization) {
            // Invalidate caches
            const { TenantService } = await import("#core/services/tenant.service");
            const { TenantStorageService } = await import("#shared/file-storage/tenant-storage.service");
            
            // Invalidate Redis/Memory cache for tenant resolution
            if (organization.tenantConfig?.customDomain) {
                 await TenantService.invalidateCache(organization.tenantConfig.customDomain);
            }
