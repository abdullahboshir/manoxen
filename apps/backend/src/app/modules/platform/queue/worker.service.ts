...
import { AutomationService } from "#app/modules/platform/automation/automation.service";
import type { IAutomationJobPayload, IEmailJobPayload, IMaintenanceJobPayload } from "./queue.interface";
import { MailService } from "#shared/mail/mail.service";
import { MailTemplates } from "#shared/mail/mail.templates";
...
const processEmailJob = async (job: Job) => {
    ...
    console.log(`ðŸ“¨ Processing Email Job [${job.id}]: To ${to} [${template || 'Raw'}]`);
...
const processMaintenanceJob = async (job: Job) => {
    ...
    console.log(`ðŸ› ï¸ Executing Maintenance Task: ${taskName} [${job.id}]`.cyan);
    ...
    try {
        ...
        console.error(`âŒ Maintenance Task Failed: ${taskName}:`.red, error.message);
...
const initWorkers = () => {
    ...
    if (emailQueue) {
        ...
        emailQueue.on('failed', (_job, err) => {
            console.error(`âŒ Job ${_job.id} failed: ${err.message}`);
        });
    }
    ...
    if (automationQueue) {
        ...
        automationQueue.on('failed', (job, err) => {
            console.error(`âŒ Job ${job.id} failed: ${err.message}`);
        });
    }
    ...
    if (maintenanceQueue) {
        ...
        maintenanceQueue.on('failed', (job, err) => {
            console.error(`âŒ Maintenance Job ${job?.id} failed: ${err.message}`.red);
        });
    }

    console.log('ðŸ‘· Workers Initialized for: Email, Automation, Maintenance');
};
