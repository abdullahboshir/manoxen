...
import { USER_STATUS } from "@manoxen/iam-core";
import { User } from "..//user/user.model";
import "../../platform/organization/business-unit/core/business-unit.model";
import "../role/role.model";
import "../permission/permission.model";
import "../permission-group/permission-group.model";
...
export const authMeService = async (
  ...
  if (res) {
    ...
    try {
      ...
      if (businessAccessInfo.primary?.organization) {
        (res as any).organization = businessAccessInfo.primary.organization;
      }
  ...
  */
  ...
  try {
    ...
    await MailService.sendEmail(
      email,
      "Setup your Manoxen Account (Link Resent)",
      `
      <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px;">
        ...
        <p>Hello,</p>
        <p>You have requested a new link to set up your password for <strong>Manoxen</strong>.</p>
        ...
        <p style="font-size: 12px; color: #94a3b8; text-align: center;">Â© 2026 Manoxen. All rights reserved.</p>
      </div>
      `
    );
  } catch (error) {
