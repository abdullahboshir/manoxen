...
import type { IRole } from "@manoxen/iam-core";
...
RoleSchema.pre('save', async function (next) {
  ...
  if (doc.isDefault) {
    ...
    if (existingDefault) {
      throw new Error(`There is already a default role for hierarchy level ${doc.hierarchyLevel}`);
    }
  }
  next();
});
...
export const Role = model<IRole>('Role', RoleSchema as any);
