import client from "#shared/config/redis.config";
import { buildUserPermissionsKey } from "#core/utils/cacheKeys";
import type { 
  IUser,
  IPermission,
...
import { CacheManager as _CacheManager } from "@manoxen/core-util";
...
export class PermissionService {
  /* ---------------------------------------------------------------------- */
  /* 1ï¸âƒ£  PUBLIC API â€“ checkPermission                                        */
  /* ---------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------- */
  /* 2ï¸âƒ£  PUBLIC API â€“ getUserPermissions (cached)                           */
  /* ---------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------- */
  /* 2.2ï¸âƒ£  PUBLIC API â€“ getUniqueResources (For Frontend Sidebar)              */
  /* ---------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------- */
  /* 2.5ï¸âƒ£  PUBLIC API â€“ getAuthorizationContext (Effective Limits & Roles)   */
  /* ---------------------------------------------------------------------- */
  async getAuthorizationContext(
    ...
    // Use organizationId/organizationId from targetScope if provided, otherwise derive it from Business Unit
    let activeOrganizationId: string | null = targetScope?.organizationId?.toString() || targetScope?.organizationId?.toString() || null;
    ...
    if (!activeOrganizationId && targetBuId && Array.isArray(user.businessAccess)) {
      ...
      if (buAccess && (buAccess.organization || buAccess.organization)) {
        activeOrganizationId = (buAccess.organization?._id || buAccess.organization?.id || buAccess.organization?._id || buAccess.organization?.id || buAccess.organization || buAccess.organization).toString();
      }
    }
    ...
    if (Array.isArray(user.businessAccess)) {
      user.businessAccess.forEach(assignment => {
        ...
        const organizationId = assignment.organization
          ...
          : (assignment.organization ? ((assignment.organization as any)._id || assignment.organization).toString() : null);
        ...
        // 2. ORGANIZATION or ORGANIZATION Scope roles include if they match the active organization
        else if ((assignment.scope === 'ORGANIZATION' || assignment.scope === 'ORGANIZATION') && activeOrganizationId && organizationId === activeOrganizationId) {
          includeRole = true;
        }
    ...
    // 3ï¸âƒ£ Derive Data Scope from Permissions (if higher than override)
  ...
  /* ---------------------------------------------------------------------- */
  /* 3ï¸âƒ£  PRIVATE helpers â€“ matching, condition evaluation, resolution        */
  /* ---------------------------------------------------------------------- */

  /**
   * Filter and Match permissions based on:
   * 1. Resource & Action match
   * 2. Golden Rules (Platform â‰  Business)
  ...
  /** get nested value like `request.user.id` â†’ `user.id` */
  ...
  private compareValues(contextValue: any, operator: string, permissionValue: any): boolean {
    switch (operator) {
      ...
      default:
        // unknown operator â†’ treat as NOT matching, but log for debugging
  ...
  private resolvePermissions(
    ...
    if (!permissions.length) {
      logger.debug('No matching permission â€“ default deny');
      return { allowed: false, reason: 'No matching permissions found' };
    }

    // 0ï¸âƒ£ Sort by Priority (High to Low)
    ...
    // 1ï¸âƒ£ explicit deny (securityâ€‘first WITHIN same priority)
    ...
    // 3ï¸âƒ£ default deny
    logger.debug('No explicit allow â€“ default deny');
    return {
      allowed: false,
...
/* -------------------------------------------------------------------------- */
/* 4ï¸âƒ£  Export singleton â€“ can be replaced with DI container if needed           */
/* -------------------------------------------------------------------------- */
