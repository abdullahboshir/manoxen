...
import { type IUser, USER_ROLE, USER_STATUS } from "@manoxen/iam-core";
import { genereteCustomerId } from "./user.utils";

import { User } from "./user.model";
import { Role } from "..//role/role.model";
import { UserBusinessAccess } from "..//user-business-access/user-business-access.model";
...
import { BusinessUnit, Staff } from "#app/modules/platform/index";
import type { IStaff } from "#app/modules/platform/index";
import { sendImageToCloudinary } from "#core/utils/file-upload";

import { AppError, QueryBuilder } from "@manoxen/core-util";
import mongoose from "mongoose";
import crypto from "crypto";
import type { ICustomer } from "#app/modules/contacts/index";
import { MailService } from "#shared/mail/mail.service";
import { Customer } from "#app/modules/contacts/index";
import { Merchant } from "#app/modules/platform/merchant/merchant.model";
...
// Re-export from Contacts module to maintain legacy compatibility
export { createCustomerService } from "#app/modules/contacts/index";

const _deprecated_createCustomerService = async (
...
export const updateUserService = async (
  ...
  if (payload.directPermissions && payload.directPermissions.length > 0) {
    const { Permission } = await import("../permission/permission.model"); // Dynamic import to avoid circular dep
...
export const createOrganizationOwnerService = async (
  ...
  try {
    ...
    // Suppress error so organization creation doesn't fail, but log it critical
  }
