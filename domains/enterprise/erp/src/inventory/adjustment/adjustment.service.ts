...
import { InventoryAdjustment } from "./adjustment.model";
import type { IInventoryAdjustment } from "./adjustment.model";
import AppError from '#shared/errors/app-error';
import { addLedgerEntryService } from '../ledger/ledger.service';
        ...
        if (adjustment.items && adjustment.items.length > 0) {
            for (const item of adjustment.items) {
                ...
                if (adjustment.outlet) {
                    ...
                    let outletEntry = inventory.outletStock?.find((os: any) => os.outlet.toString() === adjustment.outlet!.toString());
