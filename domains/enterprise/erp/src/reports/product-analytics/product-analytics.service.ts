...
import type { IProductAnalytics, IProductAnalyticsSummaryDocument } from "./product-analytics.interface";
...
productAnalyticsSchema.statics['updateProductSummary'] = async function(productId: Schema.Types.ObjectId): Promise<void> {
  ...
  const ProductReviewsSummary = model('ProductReviewsSummary');
  const reviewsSummary = await ProductReviewsSummary.findOne({ product: productId });
