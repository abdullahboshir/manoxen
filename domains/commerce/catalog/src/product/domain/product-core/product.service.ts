...
export const createProductService = async (payload: any, user?: any) => {
  ...
  try {
    ...
    if (!payload.company) {
        ...
        if (payload.businessUnit) {
             // Dynamic import to avoid potential circular dep if any, though likely safe. 
             // Using top-level import is better standard. 
             // Assuming I added top-level import in previous step or will add now.
             // I will add the logic here assuming import exists or I will used inline require/import if needed.
             // Let's use the same pattern as others.
    ...
    // b. Inventory
    const inventory = await Stock.create([{ ...payload.inventory, product: productId }], { session });
...
import { ProductVariant } from "../../features/product-variant/product-variant.model";
import { Product } from "./product.model";

import { ProductRepository } from "./product.repository";
import { ProductDetails } from "../../features/product-details/product-details.model";
import { AppError, QueryBuilder, resolveBusinessUnitId, resolveBusinessUnitQuery, CacheManager } from "@manoxen/core-util";
import { ProductShipping } from "../../features/product-shipping/product-shipping.model";
import { ProductWarrantyReturn } from "../../features/product-warranty-return/product-warranty-return.model";
import { Category } from "@manoxen/catalog/category/category.model";
import { Stock } from "../../features/inventory/stock.model";
import { ProductPricing } from "../../features/product-pricing/product-pricing.model";
