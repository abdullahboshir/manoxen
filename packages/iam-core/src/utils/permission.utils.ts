import type { IPermission, IPermissionContext } from "../types/permission.types";
...
export function buildContextFromRequest(req: any): IPermissionContext {
  ...
  return {
    ...
    resource: {
      ...
      organizationId: req.body?.organizationId || req.params.organizationId || req.headers?.['x-organization-id'],
...
export function matchesScope(
  ...
  switch (permission.scope) {
    ...
    case 'organization':
      ...
      return context.user.organizations?.some(
        (orgId: string) => orgId === context.resource?.organizationId
      ) ?? false;
    ...
    case 'branch':
      return context.user.branches?.some(
        (branch: string) => context.resource?.region === branch
      ) ?? false;
