...
export const queryContext = () => {
    return (req: Request, _res: Response, next: NextFunction) => {
        ...
        const targetOrganization = req.params?.['organizationId'] || req.headers?.['x-organization-id'] || req.query?.['organization'];
        ...
        if (targetOrganization) {
            req.query['organization'] = targetOrganization as any;
        }
        if (targetOutlet) {
        ...
        if (!targetBU && !targetOrganization && !targetOutlet && req.method === 'GET') {
            const authorizedBUs = user.businessUnits || [];
            const authorizedOrganizations = user.organizations || [];
            ...
            if (authorizedOrganizations.length > 0) {
                const organizationIds = authorizedOrganizations.map((c: any) => c._id?.toString() || c.id?.toString() || c.toString());
                // Only inject organization if BU filtering is not already narrow enough? 
                // Better to inject both to be safe (AND condition in QueryBuilder)
                req.query['organization'] = organizationIds.length === 1 ? organizationIds[0] : ({ $in: organizationIds } as any);
            }
