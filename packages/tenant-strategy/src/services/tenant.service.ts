import type { ITenantConfig } from "@manoxen/shared-types";
import { type Model } from "mongoose";
import { redisClient } from "@manoxen/platform-core";
...
export class TenantService {
    ...
    static async resolveTenant(options: {
        ...
        if (host) {
            ...
            const { Organization } = await import("@manoxen/organization");
            const orgByDomain = await (Organization as any).findOne({
            ...
            if (subdomain) {
                ...
                const { Organization } = await import("@manoxen/organization");
                const orgBySlug = await (Organization as any).findOne({
                    slug: subdomain,
                    isActive: true
                }).select('+tenantConfig.databaseUri').lean();
        ...
        if (organizationId) {
            ...
            const { Organization } = await import("@manoxen/organization");
            const orgById = await (Organization as any).findById(organizationId)
    ...
    static async invalidateCache(organizationId: string): Promise<void> {
        ...
        try {
            if (redisClient.isOpen) {
                ...
                const firstKey = keysToDelete[0];
                if (firstKey) {
                    await redisClient.del(firstKey); // Only delete known key
                }
            }
        } catch {
