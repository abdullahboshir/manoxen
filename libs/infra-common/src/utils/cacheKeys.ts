import { redisClient as client } from "@manoxen/platform-core";
...
export async function buildUserPermissionsKey(
  userId: string,
  scope?: { businessUnitId?: string | undefined; outletId?: string | undefined; organizationId?: string | undefined }
): Promise<string> {
  ...
  } else if (scope?.organizationId || scope?.organizationId) {
    scopeKey = `org:${scope.organizationId || scope.organizationId}`;
  }
