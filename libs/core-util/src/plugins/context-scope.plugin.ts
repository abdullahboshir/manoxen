...
export interface ContextScopePluginOptionsInterface {
  /**
  ...
  /**
   * Field name for organization reference (default: 'organization')
   */
  organizationField?: string;

  /**
   * Field name for business unit reference (default: 'businessUnit')
   */
  businessUnitField?: string;

  /**
   * Field name for outlet reference (default: 'outlet')
   */
  outletField?: string;
...
const DEFAULT_OPTIONS: ContextScopePluginOptionsInterface = {
  requireOutlet: false,
 ...
 */
export const contextScopePlugin = (
  schema: Schema<any, any, any, any, any>,
  options: ContextScopePluginOptionsInterface = {},
) => {
  ...
  const applyContextFilter = function (
    this: any,
    next: (err?: CallbackError) => void,
  ) {
    // ðŸ”“ BYPASS: Allow explicit bypass for system/auth queries (Phase 8 Hardening)
    ...
    if (!ctx) {
      // ðŸ›¡ï¸ READ GUARD: Zero Trust (Master Checklist Phase 2)
      if (process.env["NODE_ENV"] === "production") {
        ...
        return next(
          new Error(
            `[ContextScopePlugin] CRITICAL: Attempted to query ${schema.get("collection")} without request context. Access Denied.`,
          ),
        );
      }
    ...
    const orgId =
      contextFilter["organization"] || contextFilter["organization"];
    if (orgId) {
      filter[opts.organizationField!] = opts.includeGlobal
        ? { $in: [orgId, null] }
        : orgId;
    }
    if (contextFilter["businessUnit"]) {
      const buVal = contextFilter["businessUnit"];
      filter[opts.businessUnitField!] = opts.includeGlobal
        ? { $in: [buVal, null] }
        : buVal;
    }
    if (contextFilter["outlet"]) {
      const outletVal = contextFilter["outlet"];
      filter[opts.outletField!] = opts.includeGlobal
        ? { $in: [outletVal, null] }
        : outletVal;
    }

    // ðŸ§¬ DOMAIN ISOLATION: Pharmacy vs Grocery, etc. (Industrial Standard)
  ...
  schema.pre("aggregate", function (this: any, next) {
    // ðŸ”“ BYPASS: Allow explicit bypass for system/auth aggregations
    if (this._bypassContext) {
      return next();
    }
    ...
    const ctx = ContextService.getContext();
    if (!ctx) {
      // ðŸ›¡ï¸ AGGREGATE GUARD: Zero Trust (Master Checklist Phase 6)
      if (process.env["NODE_ENV"] === "production") {
        ContextService.logSecurityAlert({
          type: "UNAUTHORIZED_READ",
          severity: "HIGH",
          ...
          action: "AGGREGATE",
          details:
            "Attempted to aggregate without request context. Access Denied.",
        });
        return next(
          new Error(
            `[ContextScopePlugin] CRITICAL: Attempted to aggregate ${schema.get("collection")} without request context. Access Denied.`,
          ),
        );
      }
    ...
    if (contextFilter["businessUnit"])
      filter[opts.businessUnitField!] = contextFilter["businessUnit"];
    if (contextFilter["outlet"])
      filter[opts.outletField!] = contextFilter["outlet"];
  ...
  schema.pre("save", function (next: (err?: CallbackError) => void) {
    ...
    // ðŸ›¡ï¸ WRITE GUARD: Absolute Data Integrity
    if (!ctx) {
      ...
      if (process.env["NODE_ENV"] === "production") {
        return next(
          new Error(
            `[ContextScopePlugin] CRITICAL: Attempted to save ${schema.get("collection")} without request context. Access Denied.`,
          ),
        );
      }
      return next();
    ...
    enforceStamping("organizationField", "organization");
    enforceStamping("businessUnitField", "businessUnit");
    enforceStamping("outletField", "outlet");
    ...
    if (doc.isNew) {
      if (opts.requireOutlet && !stamp["outlet"]) {
        return next(
          new Error(
            `[ContextScopePlugin] Validation Error: Outlet-scoped model requires outlet context`,
          ),
        );
      }
      if (opts.requireBusinessUnit && !stamp["businessUnit"]) {
        return next(
          new Error(
            `[ContextScopePlugin] Validation Error: BusinessUnit-scoped model requires BU context`,
          ),
        );
      }
    }
    ...
    if (!doc.isNew && ctx.scopeLevel !== "PLATFORM") {
      const isGlobal =
        !(doc as any)[opts.organizationField!] &&
        !(doc as any)[opts.businessUnitField!] &&
        !(doc as any)[opts.outletField!];
      if (isGlobal) {
        ContextService.logSecurityAlert({
          ...
          action: "WRITE_GLOBAL",
          details:
            "Tenant attempted to modify a global/platform record. Operation Aborted.",
        });
        return next(
          new Error(
            `[ContextScopePlugin] CRITICAL: You are not authorized to modify global platform records.`,
          ),
        );
      }
    }
  ...
  schema.pre("insertMany", function (next, docs) {
    ...
    if (!ctx) {
      // ðŸ” WRITE GUARD: Industrial Lockdown (Master Checklist Phase 3)
      if (process.env["NODE_ENV"] === "production") {
        ContextService.logSecurityAlert({
          type: "UNAUTHORIZED_WRITE",
          severity: "CRITICAL",
          ...
          action: "WRITE_BULK",
          details:
            "Attempted to write bulk documents without request context. Operation Aborted.",
        });
        return next(
          new Error(
            `[ContextScopePlugin] CRITICAL: Attempted to write to ${schema.get("collection")} without request context.`,
          ),
        );
      }
      return next();
    ...
    for (const doc of docs) {
      ...
      enforceBulkStamping("organizationField", "organization");
      enforceBulkStamping("businessUnitField", "businessUnit");
      enforceBulkStamping("outletField", "outlet");

      // Validate
      if (opts.requireOutlet && !stamp["outlet"]) {
        return next(
          new Error(
            `[ContextScopePlugin] Bulk Validation Error: Outlet-scoped model requires outlet context`,
          ),
        );
      }
      if (opts.requireBusinessUnit && !stamp["businessUnit"]) {
        return next(
          new Error(
            `[ContextScopePlugin] Bulk Validation Error: BusinessUnit-scoped model requires BU context`,
          ),
        );
      }
    }
