...
export class ContextService {
  ...
  static getContextStamp(): Record<string, any> {
    ...
    const orgId = ctx.organizationId || ctx.organizationId;
    if (orgId) {
      stamp["organizationId"] = orgId;
      stamp["organizationId"] = orgId; // Backward compatibility for field name
    }
  ...
  /**
   * Validates that the provided ID(s) belong to the current organization context.
  ...
  static async validateReferentialIntegrity(
    ...
    const count = await model.countDocuments({
      ...
      $or: [{ organizationId: orgId }, { organizationId: orgId }],
    });

    if (count !== normalizedIds.length) {
      // Note: SecurityAlert logging removed to decouple from app-layer models.
      // In a real scenario, emit an event or use a generic logger here.
      console.error(
        `[ContextService] Security Breach: One or more referenced records in ${model.modelName} do not belong to your organization.`,
      );
      throw new Error(
        `[ContextService] Security Breach: One or more referenced records in ${model.modelName} do not belong to your organization.`,
      );
  ...
  /**
   * Log security alerts (stub method - can be extended for actual logging)
   */
  static logSecurityAlert(alert: any): void {
    console.warn("[SECURITY_ALERT]", alert);
    // In production, this would log to a security event system
  }
}
