...
export const resolveBusinessUnitId = async (
    ...
    if (identifier instanceof mongoose.Types.ObjectId) {
        ...
        if (isObjectId) {
            ...
            if (!buDoc) {
                throw new Error(`Business Unit Not Found: '${idStr}'`);
            }
    ...
    if (verifiedUser && !verifiedUser.roleName?.includes('super-admin') && !verifiedUser.isSuperAdmin && !verifiedUser.roleName?.includes('organization-owner')) {
        ...
        if (!hasAccess) {
            throw new Error("Ownership Security Error: You do not have permission for this Business Unit.");
        }
    }
